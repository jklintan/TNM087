...
% based on the variable LevelRange

%Lowest and highest intensity levels in range
...
% Use the Mask to mask out the areas within this selected IR-range in the RGB-images
% (for all 3 color channels)

RGB_Range(:,:,1) = RGB_Image(:,:,1).*Mask; %Apply the mask for R-channnel
RGB_Range(:,:,2) = RGB_Image(:,:,2).*Mask; %G-channel
RGB_Range(:,:,3) = RGB_Image(:,:,3).*Mask; %B-channel
...
% Marking just a single pixel is difficult to see in the image. Use at least a 5x5 pixel area
...
IR_Select(y-10:y+10,x-3:x+3, 1) = 1; %Creating a red cross to mark the pixels
IR_Select(y-3:y+3,x-10:x+10, 2) = 0;
IR_Select(y-10:y+10,x-3:x+3, 2) = 0;
IR_Select(y-3:y+3,x-10:x+10, 3) = 0;
IR_Select(y-10:y+10,x-3:x+3, 3) = 0; %Setting the other channels to 0

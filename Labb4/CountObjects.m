...
level = graythresh(bgray); %Otsu's algorithm
...
smallMacim=zeros(r,c);
for n=1:length(SmallMacs)
    smallMacim(L == SmallMacs(n))=1;
end

bigMacim=zeros(r,c);
for n=1:length(LargeMacs)
    bigMacim(L == LargeMacs(n))=1;
end

IMG = zeros(r, c, 3);
IMG = cat(3, Riceim, smallMacim, bigMacim);  % Output RGB-image displaying the three classes of objects in different colors

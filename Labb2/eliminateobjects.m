...
n_formula = floor(q * sqrt((4*(Q-B))/(Q-4*B))); % The size of the required filter
...
o_thresh = im2bw(im_filt, Q/4); % The result of thresholding the filtered image
...
%We want the places where the image is not 0.1 (B) and the mask is equal to
%0

mask = (im~=B & o_thresh==0); % the mask
...
output = mask.*o_thresh + (~mask.*im); % the output image
...
% Your answer here:
%
% The smallest odd sized kernel that we can use for a 2x2 object
% is a 7x7 kernel according to the formula we derived above. If a small
% 2x2 object is close to another, the kernel will contain some values of
% that other object as well. That means that the average value in the
% filter kernel will be bigger than Q/4 and it will determine that the
% object is larger than it actual is and therefor not deleting it.
%